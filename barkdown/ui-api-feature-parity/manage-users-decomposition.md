...
- IDs follow `MU-<TYPE>-NN`: `D` displayed, `S` state, `T` transition, `A` action. `MU` prefix for Manage Users.
...
|MU-S-11|Row action in progress (lock/unlock)          |User tapped lock/unlock button on a row                                            |Row shows inline loading indicator. Button on that row temporarily disabled. Other rows remain interactive.                                                                 |
|MU-S-12|Row action error                              |Lock/unlock action rejected                                                        |❓ Inline row error or toast? Design unclear. Defaulting to toast pattern per Roxbury convention.                                                                            |
...
|MU-T-04|MU-S-02                                      |MU-S-03                            |User selects business from dropdown                                                                                                                                                   |
...
|MU-T-08|MU-S-08                                      |MU-S-03                            |User taps retry                                                                                                                                                                       |
|MU-T-09|MU-S-06 (or MU-S-05)                         |MU-S-07                            |User changes business selection from dropdown                                                                                                                                         |
...
|MU-T-13|MU-S-06 (or MU-S-09)                         |MU-S-11                            |User taps lock/unlock button on a row                                                                                                                                                 |
|MU-T-14|MU-S-11                                      |MU-S-06 (with row state updated)   |Lock/unlock resolves successfully                                                                                                                                                     |
|MU-T-15|MU-S-11                                      |MU-S-12                            |Lock/unlock rejects                                                                                                                                                                   |
|MU-T-16|MU-S-12                                      |MU-S-06                            |User dismisses error                                                                                                                                                                  |
|MU-T-17|MU-S-04 or later                             |MU-S-13                            |User taps Filters button                                                                                                                                                              |
|MU-T-18|MU-S-13                                      |Previous state, with filtered fetch|User applies filters and dismisses                                                                                                                                                    |
|MU-T-19|MU-S-13                                      |Previous state                     |User dismisses filters without applying                                                                                                                                               |
|MU-T-20|MU-S-06 (or any with Add User enabled)       |MU-S-14                            |User taps Add User                                                                                                                                                                    |
...
- ❓ Q1: **Locked vs. deactivated** — are these the same status or distinct concepts? If a user is “locked” do they appear in the active table with locked status, or move to the deactivated table? This fundamentally changes the data model and the meaning of the lock/unlock button. (MU-T-23, MU-D-10, MU-D-12, MU-D-18.)
...
- ❓ Q3: **Filters dialog** — entire interaction is undesigned. What fields are filterable? Modal or panel? Multi-select or single? (MU-D-04, MU-S-13.)

**Important (resolvable in working sessions):**

- ❓ Q4: Business selector persistence across navigation. (MU-D-03)
- ❓ Q5: Pagination strategy (offset vs. cursor) and page size. (MU-D-13)
- ❓ Q6: Sort behavior — which columns, server-side or client-side. (MU-D-07)
- ❓ Q7: Deactivated users — separate endpoint, separate query, or filtered on the same response? (MU-S-09, MU-T-12)
- ❓ Q8: Permission gating on lock/unlock — role-restricted or universally available? Where do permissions come from (current user object, per-row in response)? (MU-D-12)
- ❓ Q9: Primary contact / owner chips — mutually exclusive or stackable? Source field naming. (MU-D-08)
- ❓ Q10: User ID — system ID, external ID, both? Display format, copyability. (MU-D-09)
- ❓ Q11: Last activity for users who have never logged in — display “Never”, “—”, or hide? (MU-D-11)
- ❓ Q12: Lock/unlock button behavior for `pending_setup` status. (MU-D-12)
- ❓ Q13: Loading-during-refresh UX — skeleton or stale-with-indicator. (MU-S-07)
- ❓ Q14: Row action error UX — inline or toast. (MU-S-12)
- ❓ Q15: Error state placement — inline replacing table, or banner above. (MU-D-17)

**Low priority (defaults are fine, confirm later):**

- ❓ Q16: Final copy for all static labels (title, subtitle, section headers, empty state).
- ❓ Q17: Whether tapping a user row navigates to a detail view.
- ❓ Q18: Chip color mapping for status values.
...
- **Q4–Q15** → matrix rows for the API-dependent ones (Q5, Q6, Q7, Q8, Q9, Q10), risk rows for the design-dependent ones (Q11, Q12, Q13, Q14, Q15).
- **Q16–Q18** → notes only; resolve in design review.
...
These four findings alone justify the time spent on this decomposition.
